...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $stmt = $conn->prepare("SELECT trainer_id, name, password, status FROM gym_trainers WHERE email = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows == 1) {
            $stmt->bind_result($trainer_id, $name, $hashed_password, $status);
            $stmt->fetch();
...
<head>
    ...
    <title>Trainer Login - FitZone Pro</title>
        ...
        <div class="login-card">
            ...
            <form method="post" novalidate>
                ...
                <div class="form-links">
                    <a href="trainer_forgot_password.php" class="forgot-password">
                        <i class="fas fa-key"></i> Forgot Password?
                    </a>
                    New trainer? <a href="trainer_register.php">Register here</a>
                </div>
            </form>
        </div>
    </div>
